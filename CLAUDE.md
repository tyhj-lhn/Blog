...
bash deploy.sh                                              # Interactive step-by-step
bash deploy.sh --check                                      # Dry-run: check prerequisites only
bash deploy.sh --yes --domain example.com --git-repo ...    # Non-interactive (CI/CD)
```
...
| 文件 | 说明 |
|------|------|
| [deploy.sh](deploy.sh) | 1226 行 Shell 脚本 — 14 步交互式部署 |
...
**Production config:** `ecosystem.production.config.cjs` — auto-generated by `deploy.sh` step 10 at deploy time. Contains actual JWT secrets + DB password (chmod 600). Git-ignored.
...
- `deploy.sh` generates `ecosystem.production.config.cjs` with actual secrets from `backend/.env` — no manual config needed
...
- Full deployment flow: run `bash deploy.sh` on the server → follow 14-step prompts → done
